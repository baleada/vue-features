...
suite(`binds static values to styles`, async ({ puppeteer: { page } }) => {
  await page.goto('http://localhost:3000/bindStyle/static')
...
suite(`binds dynamic values to styles`, async ({ puppeteer: { page } }) => {
  await page.goto('http://localhost:3000/bindStyle/dynamic')
...
suite(`binds values via getValue to properties on arrays of elements`, async ({ puppeteer: { page } }) => {
  await page.goto('http://localhost:3000/bindStyle/getValueArray')
  await page.waitForSelector('span')
